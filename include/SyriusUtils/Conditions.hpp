...
namespace Syrius::Priv {
    ...
    void formatAssert(const char* condition, const char* file, const char* func, int line, char const* const message, Args... args){
        std::size_t size = snprintf(nullptr, 0, message, args...);
        std::vector<char> buffer(size + 1);
        snprintf(buffer.data(), size + 1, message, args...);

        printf("[ASSERT]: [%s : %s : %i]: Condition: %s failed: %s\n", file, func, line, condition, buffer.data());
